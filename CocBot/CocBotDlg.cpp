...
void CcocBotDlg::SaveConfig(CString fileName)
{
	...
	//2.训练设置
	SetConfig2("训练设置", "TrainArmyStyle", train.TrainArmyStyle.GetCurSel(), fileName);
	SetConfig2("训练设置", "IsClearArmy", train.IsClearArmy.GetCurSel(), fileName);
	SetConfig2("训练设置", "IsAddArmy", train.IsAddArmy.GetCurSel(), fileName);
	train.Barbarin.GetWindowText(winText);
	SetConfig1("训练设置", "Barbarin", winText, fileName);
	train.Archer.GetWindowText(winText);
	SetConfig1("训练设置", "Archer", winText, fileName);
	train.Giant.GetWindowText(winText);
	SetConfig1("训练设置", "Giant", winText, fileName);
	train.Goblin.GetWindowText(winText);
	SetConfig1("训练设置", "Goblin", winText, fileName);
	train.WallBreaker.GetWindowText(winText);
	SetConfig1("训练设置", "WallBreaker", winText, fileName);
	train.Ballon.GetWindowText(winText);
	SetConfig1("训练设置", "Ballon", winText, fileName);
	train.Wizard.GetWindowText(winText);
	SetConfig1("训练设置", "Wizard", winText, fileName);
	train.Healer.GetWindowText(winText);
	SetConfig1("训练设置", "Healer", winText, fileName);
	train.Dragon.GetWindowText(winText);
	SetConfig1("训练设置", "Dragon", winText, fileName);
	train.Peka.GetWindowText(winText);
	SetConfig1("训练设置", "Peka", winText, fileName);
	train.BabyDragon.GetWindowText(winText);
	SetConfig1("训练设置", "BabyDragon", winText, fileName);
	train.Miner.GetWindowText(winText);
	SetConfig1("训练设置", "Miner", winText, fileName);
	train.Minion.GetWindowText(winText);
	SetConfig1("训练设置", "Minion", winText, fileName);
	train.HogRider.GetWindowText(winText);
	SetConfig1("训练设置", "HogRider", winText, fileName);
	train.Valkyrie.GetWindowText(winText);
	SetConfig1("训练设置", "Valkyrie", winText, fileName);
	train.Golem.GetWindowText(winText);
	SetConfig1("训练设置", "Golem", winText, fileName);
	train.Witch.GetWindowText(winText);
	SetConfig1("训练设置", "Witch", winText, fileName);
	train.LavaHound.GetWindowText(winText);
	SetConfig1("训练设置", "LavaHound", winText, fileName);
	train.BowLer.GetWindowText(winText);
	SetConfig1("训练设置", "BowLer", winText, fileName);
	train.Yeti.GetWindowText(winText);
	SetConfig1("训练设置", "Yeti", winText, fileName);
	train.ElectroDragon.GetWindowText(winText);
	SetConfig1("训练设置", "ElectroDragon", winText, fileName);
	train.IceGolem.GetWindowText(winText);
	SetConfig1("训练设置", "IceGolem", winText, fileName);
	train.Headhunter.GetWindowText(winText);
	SetConfig1("训练设置", "Headhunter", winText, fileName);
	train.LightingSpell.GetWindowText(winText);
	SetConfig1("训练设置", "LightingSpell", winText, fileName);
	train.HealingSpell.GetWindowText(winText);
	SetConfig1("训练设置", "HealingSpell", winText, fileName);
	train.RageSpell.GetWindowText(winText);
	SetConfig1("训练设置", "RageSpell", winText, fileName);
	train.JumpSpell.GetWindowText(winText);
	SetConfig1("训练设置", "JumpSpell", winText, fileName);
	train.FreezeSpell.GetWindowText(winText);
	SetConfig1("训练设置", "FreezeSpell", winText, fileName);
	train.CloneSpell.GetWindowText(winText);
	SetConfig1("训练设置", "CloneSpell", winText, fileName);
	train.PoisonSpell.GetWindowText(winText);
	SetConfig1("训练设置", "PoisonSpell", winText, fileName);
	train.EarthquakeSpell.GetWindowText(winText);
	SetConfig1("训练设置", "EarthquakeSpell", winText, fileName);
	train.HasteSpell.GetWindowText(winText);
	SetConfig1("训练设置", "HasteSpell", winText, fileName);
	train.SkeletonSpell.GetWindowText(winText);
	SetConfig1("训练设置", "SkeletonSpell", winText, fileName);
	SetConfig2("训练设置", "IsSpeedUp", train.IsSpeedUp.GetCheck(), fileName);
	train.SpeedUpBegin.GetWindowText(winText);
...
void CcocBotDlg::UpdateWindowSet()
{
	...
	train.TrainArmyStyle.SetCurSel(_ttoi(script[0].coc.getSets("TrainArmyStyle")));
	train.IsClearArmy.SetCurSel(_ttoi(script[0].coc.getSets("IsClearArmy")));
	train.IsAddArmy.SetCurSel(_ttoi(script[0].coc.getSets("IsAddArmy")));
	train.IsSpeedUp.SetCheck(_ttoi(script[0].coc.getSets("IsSpeedUp")));
	train.SpeedUpBegin.SetWindowText(script[0].coc.getSets("SpeedUpBegin"));
	train.SpeedUpEnd.SetWindowText(script[0].coc.getSets("SpeedUpEnd"));
	train.Barbarin.SetWindowText(script[0].coc.getSets("Barbarin"));
	train.Archer.SetWindowText(script[0].coc.getSets("Archer"));
	train.Giant.SetWindowText(script[0].coc.getSets("Giant"));
	train.Goblin.SetWindowText(script[0].coc.getSets("Goblin"));
	train.WallBreaker.SetWindowText(script[0].coc.getSets("WallBreaker"));
	train.Ballon.SetWindowText(script[0].coc.getSets("Ballon"));
	train.Wizard.SetWindowText(script[0].coc.getSets("Wizard"));
	train.Healer.SetWindowText(script[0].coc.getSets("Healer"));
	train.Dragon.SetWindowText(script[0].coc.getSets("Dragon"));
	train.Peka.SetWindowText(script[0].coc.getSets("Peka"));
	train.BabyDragon.SetWindowText(script[0].coc.getSets("BabyDragon"));
	train.Miner.SetWindowText(script[0].coc.getSets("Miner"));
	train.Minion.SetWindowText(script[0].coc.getSets("Minion"));
	train.HogRider.SetWindowText(script[0].coc.getSets("HogRider"));
	train.Valkyrie.SetWindowText(script[0].coc.getSets("Valkyrie"));
	train.Golem.SetWindowText(script[0].coc.getSets("Golem"));
	train.Witch.SetWindowText(script[0].coc.getSets("Witch"));
	train.LavaHound.SetWindowText(script[0].coc.getSets("LavaHound"));
	train.BowLer.SetWindowText(script[0].coc.getSets("BowLer"));
	train.ElectroDragon.SetWindowText(script[0].coc.getSets("ElectroDragon"));
	train.Yeti.SetWindowText(script[0].coc.getSets("Yeti"));
	train.IceGolem.SetWindowText(script[0].coc.getSets("IceGolem"));
	train.Headhunter.SetWindowText(script[0].coc.getSets("Headhunter"));
	train.LightingSpell.SetWindowText(script[0].coc.getSets("LightingSpell"));
	train.HealingSpell.SetWindowText(script[0].coc.getSets("HealingSpell"));
	train.RageSpell.SetWindowText(script[0].coc.getSets("RageSpell"));
	train.JumpSpell.SetWindowText(script[0].coc.getSets("JumpSpell"));
	train.FreezeSpell.SetWindowText(script[0].coc.getSets("FreezeSpell"));
	train.CloneSpell.SetWindowText(script[0].coc.getSets("CloneSpell"));
	train.PoisonSpell.SetWindowText(script[0].coc.getSets("PoisonSpell"));
